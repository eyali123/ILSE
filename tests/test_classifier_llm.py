"""
End-to-end ILGEClassifier + LLMLayerExtractor against a small real LLM.

Marked `llm`: downloads $ILGE_TEST_MODEL (default EleutherAI/pythia-14m).
...
from ilge import ILGEClassifier, CayleyConfig, DeepSetConfig

MODEL = os.environ.get("ILGE_TEST_MODEL", "EleutherAI/pythia-14m")
...
@pytest.mark.parametrize(
    ...
    [
        ...
        DeepSetConfig(hidden_dim=32, epochs=5, batch_size=8),
    ],
    ids=["cayley", "deepset"],
)
def test_classifier_end_to_end(tmp_path, config):
    ...
    clf = ILGEClassifier(MODEL, config, num_classes=3, device=_dev())
    try:
    ...
    clf.save(str(path))
    clf2 = ILGEClassifier.load(str(path), device=_dev())
...
def test_token_pooling_variants(pooling):
    pytest.importorskip("transformers")
    from ilge._internal.llm import LLMLayerExtractor
