...
from ilge import CayleyConfig, FCConfig, DeepSetConfig
...
def test_recommended_types_and_lr():
    ...
    assert isinstance(recommended_config("deepset"), DeepSetConfig)
...
def test_suggest_deepset():
    cfg = suggest_config(FakeTrial(), "deepset")
    assert isinstance(cfg, DeepSetConfig)
