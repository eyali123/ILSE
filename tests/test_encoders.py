"""Encoder modules: DeepSetEncoder, GNNEncoder construction, real-node pooling."""
import types
...
from ilge._internal.nn_modules import DeepSetEncoder, build_gnn_encoder


# --------------------------------------------------------------------------- #
# DeepSetEncoder (no PyG needed)
# --------------------------------------------------------------------------- #

def test_deepset_shapes():
    enc = DeepSetEncoder(num_layers=7, layer_dim=16, hidden_dim=32).eval()
    y = enc(torch.randn(4, 7, 16))
    assert y.shape == (4, 32)
    assert enc.out_dim == 32


def test_deepset_no_mlps_returns_pooled_dim():
    enc = DeepSetEncoder(7, 16, hidden_dim=32, pre_pooling_layers=0, post_pooling_layers=0).eval()
    y = enc(torch.randn(2, 7, 16))
    assert y.shape == (2, 16)
    assert enc.out_dim == 16


def test_deepset_rejects_bad_pooling():
    with pytest.raises(ValueError):
        DeepSetEncoder(7, 16, pooling="last")
