...
from ilge._internal.nn_modules import DeepSetEncoder, ClassificationHead, build_gnn_encoder
from ilge._internal.dataset import (
    TensorDataset,
    GraphDataset,
    tensor_collate,
    graph_collate,
)
from ilge._internal.training import train_model

from conftest import make_synthetic


def test_train_deepset_synthetic(device):
    xs, ys = make_synthetic()
    ds = TensorDataset(xs, ys)
    loader = DataLoader(ds, batch_size=8, shuffle=True, collate_fn=tensor_collate)

    enc = DeepSetEncoder(num_layers=7, layer_dim=16, hidden_dim=32)
